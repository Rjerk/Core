...
recv_v6(int seq, struct timeval *tv)
{
	...
	for ( ; ; ) {
		...
		if (icmp6->icmp6_type == ICMP6_TIME_EXCEEDED &&
			...
			if ((size_t)icmp6len < 8 + sizeof(struct ip6_hdr) + 4)
...
recv_v4(int seq, struct timeval *tv)
{
    ...
    for ( ; ; ) {
        ...
        if (icmp->icmp_type == ICMP_TIMXCEED && icmp->icmp_code == ICMP_TIMXCEED_INTRANS) {
            if ((size_t)icmplen < 8 + sizeof(struct ip))
        ...
        for (probe = 0; probe < nprobes; probe++) {
            ...
            if ( (code = (*pr->recv)(seq, &tvrecv)) == -3)
                ...
                if (sock_cmp_addr(pr->sarecv, pr->salast, pr->salen) != 0) {
                    if (getnameinfo(pr->sarecv, pr->salen, str, (socklen_t) sizeof(str), NULL, 0, 0) == 0)
...
main(int argc, char **argv)
{
    ...
    pr->sarecv = (SA *) Calloc(1, ai->ai_addrlen);
    pr->salast = (SA *) Calloc(1, ai->ai_addrlen);
    pr->sabind = (SA *) Calloc(1, ai->ai_addrlen);
