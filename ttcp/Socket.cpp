...
void Socket::bindOrDie(const InetAddress& addr)
{
    ...
    int ret = ::bind(sockfd, sockaddr_cast(&saddr), sizeof(saddr));
    if (ret) {
...
{
    const struct sockaddr_in& saddr = addr.getSockAddrInet();
    return ::connect(sockfd, sockaddr_cast(&saddr), sizeof(saddr));
}
...
{
    int sockfd_ = ::socket(AF_INET, SOCK_STREAM | SOCK_CLOEXEC, IPPROTO_TCP);
    assert(sockfd_ >= 0);
...
{
    int sockfd_ = ::socket(AF_INET, SOCK_DGRAM | SOCK_CLOEXEC, IPPROTO_UDP);
    assert(sockfd_ >= 0);
    return Socket(sockfd_);
}
