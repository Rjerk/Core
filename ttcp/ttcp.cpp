...
void transmit(const Options& opt)
{
    ...
    if (stream->sendAll(&session_message, sizeof(session_message))
    ...
    std::unique_ptr<PayloadMessage, void (*)(void*)> free_it(payload, ::free);
    assert(payload);
...
void receive(const Options& opt)
{
    ...
    struct SessionMessage session_message = { 0, 0 };
    if (stream->receiveAll(&session_message, sizeof(session_message))
            != sizeof(session_message)) {
    ...
    printf ("receive buffer length = %d\nreceive number of buffers = %d\n",
            session_message.length, session_message.number);

    double total_mb = 1.0 * session_message.number * session_message.length / 1024 / 1024;
    printf ("%.3f MiB in total\n", total_mb);

    const int total_len = sizeof(int32_t) + session_message.length;
    PayloadMessage* payload = static_cast<PayloadMessage*>(::malloc(total_len));
    std::unique_ptr<PayloadMessage, void (*)(void*)> free_it(payload, ::free);
    assert(payload);
